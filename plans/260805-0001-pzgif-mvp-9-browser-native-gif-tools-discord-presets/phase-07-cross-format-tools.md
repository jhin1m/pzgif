...
1. Should `mp4-to-gif` accept audio-bearing files silently, or say up front that audio is dropped? The wireframe FAQ covers it. **Recommend a one-line note near the dropzone** as well — it is the second-most-asked question about this conversion.
2. What is the right frame cap for split? 500 is a starting guess. Set it from Phase 1's memory measurements per device tier rather than a constant.
3. Is `webp-to-gif` worth shipping at MVP at all given it is broken on ~20-30% of traffic? It is one of the nine ratified tools and the keyword is winnable, so ship it — but if Phase 7 runs long, this is the first thing to cut.
