...
3. Logo/mark does not exist yet; wireframes use a placeholder loop mark built from the brief in report §6. **[Amended 2026-08-07] Favicon legibility resolved, mark still provisional:** `src/app/icon.svg` ships the loop mark on a solid `primary-600` plate, rendered and checked at 16, 32 and 48px against both a light and a dark tab strip. Two things had to give at that size and are recorded in the file: the mark sits on a plate, because a hairline stroke on a transparent ground disappears into a white tab bar and again into a dark one; and its two lobes were pulled from 13 units apart to 10 with their radius grown 5.5 → 6.4, because the header mark's 24 × 11 proportion leaves roughly six device pixels of height at 16px and the counters close up. `favicon.ico` carries 16/32/48 frames, the 16 from a thinner-stroked variant. None of this settles the mark itself — when a real one lands, all three icon files are redrawn from it.
