...
4. **WebP compositing is under-tested.** The parser's offsets, blend and
   disposal flags are unit-tested against synthesised containers, but the
   *compositing* in `decode/webp.ts` is only exercised by `anim.webp`, whose 48
   frames are all full-canvas with no offsets and no disposal. A fixture with
   offset and disposed frames needs `webpmux` or `img2webp` to author. Phase 11.
